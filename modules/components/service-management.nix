...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.user.services."${cfg.session.target}-env" = {
      Unit = {
    ...
    # Post-start service
    systemd.user.services."${cfg.session.target}-post" = mkIf (cfg.session.postStartCommands != "") {
      Unit = {
    ...
    # Core services
    systemd.user.services.quickshell = mkIf cfg.core.quickshell.enable (
      createUserService {
        name = "quickshell";
    ...
    systemd.user.services.hypridle = mkIf cfg.core.hypridle.enable (
      createUserService {
        name = "hypridle";
    ...
    systemd.user.services.hyprpaper = mkIf cfg.core.hyprpaper.enable (
      createUserService {
        name = "hyprpaper";
    ...
    # Optional services
    systemd.user.services.polkit-agent = mkIf cfg.optional.polkitAgent.enable (
      createUserService {
        name = "polkit-agent";
    ...
    systemd.user.services.nm-applet = mkIf cfg.optional.networkManagerApplet.enable (
      createUserService {
        name = "nm-applet";
    ...
    systemd.user.services.blueman-applet = mkIf cfg.optional.bluetoothApplet.enable (
      createUserService {
        name = "blueman-applet";
    ...
    systemd.user.services.cliphist = mkIf cfg.optional.clipboardManager.enable (
      createUserService {
        name = "cliphist";
    ...
    # Service monitoring
    systemd.user.services.dots-hyprland-monitor = mkIf cfg.monitoring.enable {
      Unit = {
    ...
    systemd.user.services = lib.genAttrs cfg.session.autoStart (serviceName: {
      Install = {
        WantedBy = [ cfg.session.target ];
      };
    });
