...
apply_foot() {
    # Read terminal transparency directly from slider value, not global transparency mode
    if [ -f "$HOME/.cache/ags/user/generated/terminal/transparency" ]; then
        term_alpha=$(cat "$HOME/.cache/ags/user/generated/terminal/transparency")
    else
        term_alpha=100
    ...
    # Apply alpha transparency (convert percentage to decimal)
    alpha_decimal=$(echo "scale=2; $term_alpha / 100" | bc)
    sed -i "s/{{ \$alpha }}/$alpha_decimal/g" "$HOME"/.cache/ags/user/generated/foot/foot.ini
...
apply_term() {
    ...
    # Convert percentage (0-100) to decimal alpha (0.0-1.0)
    # Use raw percentage value for alpha (not decimal)
